...
// -- Key points:
// -- 1. Rpc handler functions are asynchronous, thus returning a Future of Result<Value>.
// -- 2. The call format is normalized to two `impl FromResources` arguments (for now) and one optionals  `impl IntoParams`, which represent the json-rpc's optional value.
// -- 3. `into_box` is a convenient method for converting a RpcHandler into a Boxed dyn RpcHandlerWrapperTrait, allowing for dynamic dispatch by the Router.
// -- 4. A `RpcHandler` will typically be implemented for static functions, as `FnOnce`, enabling them to be cloned with none or negligible performance impact, thus facilitating the use of RpcRoute dynamic dispatch.
// -- 5. `T` is the tuple of `impl FromResources` arguments.
// -- 6. `P` is the `impl IntoParams` argument.
// --
...
// -- Macro generating the RpcHandler implementations for zero or more FromResources with the last argument being IntoParams and one with not last IntoParams argument.
